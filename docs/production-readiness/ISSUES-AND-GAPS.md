...
| Manual keyboard + screen-reader audit | 🟡 Automated WCAG gate now covers all page templates in BOTH themes (axe-core e2e, 29 routes); manual keyboard/screen-reader walkthrough still open |
| WCAG accessibility pass (automated) | 🟢 axe-core e2e gate (`e2e/accessibility.spec.ts`): 17 light routes (now incl. category detail — a route that shipped always-visible brand-accent text at 1.09–4.49:1 undetected) + 12 dark-theme routes. Theme-aware accent system: `--acc-0..8`, `--tool-acc-*`, `--accent-ink` tokens with light/dark twins in globals.css; tools hub + calculators suite + matchmaker + category pages consume vars (color-mix for alpha tints), so every accent is ≥4.5:1 as text and as chip/tab background in both themes; `.eyebrow` and always-visible card CTAs use `--accent-ink` (immune to inline brand-colour overrides); dark `--muted` brightened (#9a9488 → #a29c90, fixes 4.35:1 10px chip). Verified: axe CLEAN on 29 routes; 51/51 e2e, 308 vitest, budget 413,959/450,000 |
