...
| Manual keyboard + screen-reader audit | 🟡 Automated WCAG gate now covers all page templates (axe-core e2e, 16 routes); manual keyboard/screen-reader walkthrough still open |
| WCAG accessibility pass (automated) | 🟢 axe-core e2e gate (`e2e/accessibility.spec.ts`, 16 routes) blocks color-contrast, landmark, and heading-order violations. Fixed at source: duplicate `Breadcrumb` landmark on the Razorpay page (island nav hidden via `showBreadcrumb={false}`), kbd hint + currency labels lost their `opacity-70` dimming, glossary "See also" links are always underlined, calculator/tool accent palettes darkened to AA-safe twins (all ≥4.5:1 as text and as tab backgrounds), `--muted`/`--success-text`/`--warning-text` tokens darkened, empty-state heading level fixed, error-page hints use `--muted-text`. Verified: axe CLEAN on 11 ad-hoc routes incl. /tools hub; 38/38 e2e (16 new), 308 vitest, budget 414,018/450,000 |
