...
| O3 | Switch RBI ingestion from local snapshot files to live fetching, then run the review queue | ✅ done (2026-08-18) — `npm run platform:fetch-rbi` pulls RBI's live PSS applications-status page (36 entries); baseline + v1 diff ran through the ingest pipeline (84 events, review queue); `platform:fetch-rbi-coa` additionally pulls the authorised CoA list (70 holders) from `PublicationsView.aspx?id=12043`, closing the authorised-baseline gap; all 48 removal items verified as diff artifacts (see `radar-coa-verification-2026-08-18.md`) |
